  ...
  {summaries.length === 0 ? (
    ...
    summaries.map((entry, i) => (
      <div key={i} className="bg-white p-4 rounded-lg border shadow-sm mb-3">
        <p className="font-semibold text-black">{entry.patientName} — {entry.bedNumber}</p>
        <p className="text-black">Nurse: {entry.nurseName}</p>
        <p className="text-black">Risk: {entry.riskLevel}</p>
        <p className="text-black">Shift: {entry.shift}</p>
        <p className="text-black">Key Changes: {entry.keyChanges}</p>
        <p className="text-black">SBAR: {entry.sbar}</p>
        <p className="text-black">Tasks: {entry.tasks}</p>
